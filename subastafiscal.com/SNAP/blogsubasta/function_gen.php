...
function fnc_busca_servicio($prm_servicio){
	...
	$query  = "select servicio, nombre_servicio, servicio_padre from df_servicios where tipo_acceso = 1 and servicio_padre = '".$prm_servicio."' order by orden";
...
function fncv_selectInput($valor,$classv,$option,$nameselect,$data){
	...
	if (is_array($data)){
		foreach ($data as $k => $v){
			...
			$htmlstring .= '<option value="'.$campo[0].'" '.$selected.'>'. fnc_rem_acentos($campo[1]).'</option>';
		}
	}
	$htmlstring .= "</select>";
	return $htmlstring;
