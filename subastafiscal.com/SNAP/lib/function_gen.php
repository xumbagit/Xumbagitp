...
function fnc_ejecutaQuery($sql){
	...
	$i = 0;
	$data = '';
	while($retorno = $dbn->db_fetch_array($result)){
...
function fncv_selectInput($valor,$classv,$option,$nameselect,$data){
	...
	if (is_array($data)){
		foreach ($data as $k => $v){
			...
			$htmlstring .= '<option value="'.$campo[0].'" '.$selected.'>'. fnc_rem_acentos(utf8_encode($campo[4])).'</option>';
		}
	}
	$htmlstring .= "</select>";
	return $htmlstring;
...
function fncv_mostrar_servicio($prm_servicio){
	...
	$retorno = fnc_busca_servicio($prm_servicio);
	if (count($retorno) == 0) return;
	
	for($i=0;$i<count($retorno);$i++){
		echo '<span class="textcheckbox">'.$retorno[$i]['nombre_servicio'].'</span>';
        echo '<input type="checkbox" class="box" name="Servicios[]"  value="'.$retorno[$i]['servicio'].'"/>';
        echo '<br>';
...
case 6:
    ...
    if (strlen(trim($_REQUEST['codreg'])) > 0 ){
        $sql = "SELECT * FROM municipios where id_estado = ".$_REQUEST['codreg'];
		if($dbn->QuerySQL($sql)==0){
...
case 8:
	//Registro - Parroquia
    if (strlen(trim($_REQUEST['codreg'])) > 0 ){
        $sql = "SELECT * FROM parroquias where id_municipio = ".$_REQUEST['codreg'];
		if($dbn->QuerySQL($sql)==0){
			//echo("HOLA");echo(USUARIO_BD);
			//echo($dbn->getFilas());
			if($dbn->getFilas()>0){
