...
<div class="modulobar" style="margin-left: 30px;">
	<form method="POST" enctype="multipart/form-data">
		...
		//error_reporting(E_ERROR);
			...
			if($btn){
				if($option=="publicidad"){
					if($suboption=="modificar")
						$logo=$_FILES["imgicono"]['name'];
						$tamano = $_FILES["imgicono"]['size'];
					    $tipo = $_FILES["imgicono"]['type'];
					    $archivo = $_FILES["imgicono"]['name'];
						$enlace=$_POST['nombrenlace'];
						if($archivo!=''){
							$sqlog="SELECT * FROM publicidad WHERE ID='".$_GET['IDX']."'";
							...
							if($archivo!=''){
						        ...
						        if(move_uploaded_file($_FILES["imgicono"]['tmp_name'],$destino)){
									$sqlog="UPDATE publicidad SET imagen='$destino' WHERE ID='".$_GET['IDX']."'";
									$pedidolog=mysql_query($sqlog,$conexion);
									if($pedidolog){
						...
						if($enlace!=''){
							$sqlog="UPDATE publicidad SET link='$enlace' WHERE ID='".$_GET['IDX']."'";
							$pedidolog=mysql_query($sqlog,$conexion);
							if($pedidolog){
								echo("Se han agregado la imagen con exito");
							}
		...
		<table>
			<tr>
				<td>
						<?php
						if($_GET['opcion']=='publicidad'){
							?>
								<a href="<?php echo("?mod=".$_GET['mod']."&opcion=publicidad&entrar=".$_GET['entrar']); ?>"><h1>PUBLICIDAD</h1></a>
							<?php
						}
						else{
							?>
		...
		?>
			...
			<tr>
				<td>
					<INPUT name="nombrenlace" id="nombrenlace" type="text" value="<?php echo($enlace); ?>">
				</td>
			</tr>
		</table>
		<table>
